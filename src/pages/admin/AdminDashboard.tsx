...
import { collection, query, onSnapshot, orderBy, limit, getDocs, doc, updateDoc, deleteDoc, where } from 'firebase/firestore';
import { 
  Users, 
  Sprout, 
  Activity, 
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-bg-gray flex">
      {/* Sidebar for Desktop */}
      <aside className={`bg-navy text-white transition-all duration-300 fixed h-full z-50 ${isSidebarOpen ? 'w-64' : 'w-20'} sm:relative`}>
        <div className="p-6 flex items-center gap-3">
          <Logo className="w-9 h-9" iconSize="h-5 w-5" />
          {isSidebarOpen && <span className="text-xl font-bold tracking-tight">Admin LB</span>}
        </div>
        ...
        <button 
          ...
          className="absolute -right-4 top-20 bg-mint text-navy p-1 rounded-full shadow-lg hidden sm:block"
        >
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto h-screen p-4 sm:p-10">
        <header className="mb-10 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-black text-navy capitalize">{activeTab.replace('_', ' ')}</h1>
            <p className="text-blue-gray font-medium mt-1">Sistem kontrol pusat LahanBersama.</p>
          </div>
          <div className="flex items-center gap-4 bg-white p-2 rounded-2xl border border-light-gray">
             ...
             <div className="pr-4">
                <p className="text-[10px] font-black tracking-widest text-blue-gray uppercase">Mode</p>
